...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "  Web" , " Term", " Files", " Code", " Chat", " Media", }, s, layouts[1])
end
-- }}}
...
-- Calendar Icon
calendaricon = wibox.widget.textbox()
calendaricon:set_text("")


-- Ram usage Icon
memicon = wibox.widget.textbox()
memicon:set_text("RAM: ")
...
-- Uptime Icon
uptimeicon = wibox.widget.textbox()
uptimeicon:set_text(" ")

-- Cpu load Icon
neticon = wibox.widget.textbox()
neticon:set_text("NET: ")


-- Textclock
mytextclock = awful.widget.textclock(markup(gray, " %a")
.. markup(blue, " %b ") .. markup(gray, "%d, ") ..  markup(blue, "%H:%M "))
...
seperatoricon = wibox.widget.textbox()
seperatoricon:set_text(" | ")
--seperatoricon:set_text(" • ")
...
vicious.register(cmus_widget, vicious.widgets.cmus,
    function (widget, args)
        if args["{status}"] == "Stopped" then
            ...
            return args["{status}"]..': '..markup(blue, args["{artist}"])..' - '..markup(blue,  args["{title}"])
        end
    end, 7)
--}}}
...
function check_eth()
  local eth_file = io.open("/sys/class/net/enp3s0/operstate", "r")
...
function eth_status()
  if (check_eth() == "up") then 
    eth_widget:set_markup(markup(blue, "ON"))
      else
    eth_widget:set_markup(markup(blue, "OFF"))
  end
end
...
eth_timer:connect_signal("timeout=60",eth_status)
eth_timer:start()
...
vicious.register(uptimewidget, vicious.widgets.uptime, "$1D $2h $3min")
...
netwidget = blingbling.net({interface = "enp3s0",
              show_text = true,
              font = "Sans",
              font_size = "9",
              text_color = "#DCDCCC",
...
for s = 1, screen.count() do
    ...
    right_layout:add(seperatoricon)
...
globalkeys = awful.util.table.join(
    ...
    --awful.key({ }, "Print", function () awful.util.spawn("scrot /media/movies/Movies/watch/scrot_%Y-%m-%d_%T.png && notify-send ' scrot ' ' taken '") end),
    awful.key({ }, "Print", function () awful.util.spawn_with_shell("scrot /media/movies/Movies/watch/scrot_%Y-%m-%d_%T.png && notify-send ' Screenshot ' ' Saved '") end),
